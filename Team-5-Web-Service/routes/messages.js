 ...
 */ 
 router.post("/", (request, response, next) => {
    ...
    let insert = `INSERT INTO Messages(ChatId, Message, MemberId)
                  VALUES($1, $2, $3) 
                  RETURNING PrimaryKey AS MessageId, ChatId, Message, MemberId AS email, to_char(Timestamp AT TIME ZONE 'PDT', 'YYYY-MM-DD HH24:MI:SS.US') AS Timestamp`
    ...
    pool.query(insert, values)
        .then(result => {
            if (result.rowCount == 1) {
                ...
                response.memberid = result.rows[0].email
    ...
    pool.query(query, values).then(result => {
        if (result.rowCount == 1) {
            console.log(result);
            // attach sender's firstname and lastname to the Response obj
        ...
        response.status(400).send({
            message: "SQL Error on getting firstname and lastname",
            error: err
        })
    })
},(request, response) => {
